...
class FirestoreData {
    ...
    constructor() {
        ...
        if (TESTMODE){ // If using test account
            uid = "tester"
        } else {
    ...
    public fun getUserExists() : Boolean  {
        // Returns if the uid user exists
        return userExists
    }

    public fun checkUserExists(){
        // If the user exists, good and update data maps and lists. Otherwise create account
    ...
    public fun updateCompletionDates(dataMap : MutableMap<String, Any>) {
        // Updates the dataMap to have the new completion dates
        db.collection(synap).document(uid)
            .collection("General").document("listOfCompletedSessions")
            .get()
    ...
    fun getTasksAsMap(tasks : MutableMap<String, Task>){
        // Gets the task as a map for display on the calendar
        db.collection(synap).document(uid)
            .collection("Tasks")
            .get()
    ...
    fun getCompletionCount(completion: (Long) -> Unit) {
        // Async call to get the completion count for trophies
        db.collection(synap).document(uid)
            .collection("General").document("CompletionCount")
            .get()
            .addOnSuccessListener { document ->
